 ...
 *
 * QCA955X DDR2/DDR1 initialization.
 * Based on QCA956X ddr.c and QSDK init-955x.c.
 *
 * The QCA955X DDR controller is register-compatible with QCA956X,
 * using the same AR71XX_DDR_CTRL_BASE and register layout.
 */
...
/* DDR control config register bits (shared with QCA956X) */
...
/* QCA955X shared DDR register offsets (same controller as QCA956X) */
...
#define QCA955X_DDR_REG_FSM_WAIT_CTRL	0xe4
#define QCA955X_DDR_REG_DDR3_CONFIG	0x15c
...
/*
 * CFG_DDR_REFRESH_VAL:
 * DDR2: 0x40c3 (25MHz) / 0x4138 (40MHz)
 * DDR1: 0x4186 (25MHz) / 0x4270 (40MHz)
 */
#define CFG_DDR2_REFRESH_VAL	0x40c3

/* DDR2 CAS=4 timing config (QCA955X default) */
#define CFG_DDR2_CONFIG_VAL \
	DDR_CONFIG_CAS_LATENCY_MSB_SET(0x1) | \
...
#define CFG_DDR2_CONFIG2_VAL \
	DDR_CONFIG2_HALF_WIDTH_LOW_SET(0x1) | \
...
#define CFG_DDR2_EXT_MODE_OCD_VAL	0x782
...
#define CFG_DDR_CTL_CONFIG \
	DDR_CTL_CONFIG_SRAM_TSEL_SET(0x1) | \
...
void qca955x_ddr_init(void)
{
	...
	u32 mod_val_init, cycle_val, tap_val, ctl_config;
	...
	writel(DDR_FSM_WAIT_CTRL_VAL, ddr_regs + QCA955X_DDR_REG_FSM_WAIT_CTRL);
	udelay(100);
	...
	writel(ddr_config3, ddr_regs + QCA955X_DDR_REG_DDR3_CONFIG);
	udelay(100);
	...
	/* CKE Enable */
	writel(ddr_config2 | 0x80, ddr_regs + AR71XX_DDR_REG_CONFIG2);
	udelay(100);
	...
	/* EMR2 */
	writel(0, ddr_regs + QCA955X_DDR_REG_DDR2_EMR2);
	writel(0x10, ddr_regs + AR71XX_DDR_REG_CONTROL);
	udelay(10);

	/* EMR3 */
	writel(0, ddr_regs + QCA955X_DDR_REG_DDR2_EMR3);
	writel(0x20, ddr_regs + AR71XX_DDR_REG_CONTROL);
	udelay(10);

	/* EMR DLL enable */
	writel(CFG_DDR2_EXT_MODE_VAL, ddr_regs + AR71XX_DDR_REG_EMR);
	udelay(100);
	...
	/* Auto Refresh x2 */
	writel(0x4, ddr_regs + AR71XX_DDR_REG_CONTROL);
	udelay(10);
	writel(0x4, ddr_regs + AR71XX_DDR_REG_CONTROL);
	udelay(10);
	...
	/* EMR OCD default */
	writel(CFG_DDR2_EXT_MODE_OCD_VAL, ddr_regs + AR71XX_DDR_REG_EMR);
	udelay(100);
	...
	writel(tap_val, ddr_regs + QCA955X_DDR_REG_TAP_CTRL2);
	writel(tap_val, ddr_regs + QCA955X_DDR_REG_TAP_CTRL3);
	...
	/* Switching regulator setting for SGMII noise reduction */
	writel(0x633c8176 & ~(7 << 1), srif_regs + QCA955X_SRIF_PMU1_REG);
	writel(0x633c8176 | (1 << 3), srif_regs + QCA955X_SRIF_PMU1_REG);
}
